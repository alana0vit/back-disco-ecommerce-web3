...
export class LimpezaReservasCronService {
  ...
  constructor(private readonly reservaEstoqueService: ReservaEstoqueService) {}
  ...
  async handleLimpezaReservasExpiradas() {
    ...
    try {
      const totalLiberado =
        await this.reservaEstoqueService.limparReservasExpiradas();
      ...
      this.logger.error(
        `Erro na limpeza de reservas: ${error.message}`,
        error.stack,
      );
    }
  }
